...
const [constiency, setconstiency] = useState("");
  ...
  const getDistrictDetails = () => {
    axios.get("http://localhost:3000/districtDetails").then((info) => {
  ...
  const handleDelete = (dist) => {
    axios.delete("http://localhost:3000/districtDetails/" + dist.id).then(() => {
      getDistrictDetails();
    });
  };
  ...
  const handleAddDistrict = () => {
    ...
    axios.post("http://localhost:3000/districtDetails/",newDistrict).then(()=>{
      setisADD(false)
      getDistrictDetails()
    })
  };
  ...
  const addConstituency = (dist)=>{
    ...
    axios.put("http://localhost:3000/districtDetails/"+dist.id,dist).then(()=>{
      getDistrictDetails()
    })
  }
  return (
            ...
            <tbody>
              {districts.map((dist,i)=>{
                return <tr>
                  ...
                  <td><button className="btn btn-danger" onClick={()=>{handleDelete(dist)}}>Delete</button></td>
                </tr>
