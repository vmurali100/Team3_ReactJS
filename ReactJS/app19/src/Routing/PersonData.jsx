...
export default class PersonData extends Component {
    ...
    addPerson = () => {
        ...
        axios.post("http://localhost:3000/person", this.state.person).then(() => {
            this.getAllUsers()
        })
    ...
    deletePerson = (pn) => {
        axios.delete("http://localhost:3000/person/" + pn.id).then((res) => {
            // let newPersons = { ...this.state.persons }
            this.clearForm()
            this.getAllUsers()
        })
        ...
        // let newPersons=[...this.state.persons]
        // this.setState({persons:newPersons.filter((p)=>p.fname!==pn.fname)})
    ...
    UpdatePerson = (i) => {
        ...
        axios.put("http://localhost:3000/person/" + allpersons[this.state.index].id, this.state.person).then((res) => {
            this.setState({persons:allpersons,index:null})            
            this.getAllUsers()
            this.clearForm()
    ...
    render() {
        return <div>
            <form>
                ...
                {this.state.index !==null ?(<button type='button' className="btn btn-primary" onClick={this.UpdatePerson}>update data</button>
